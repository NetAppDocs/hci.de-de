...
NOTE: Bearbeiten Sie den NTP nicht, nachdem Sie die Zeitsynchronisierungskonfiguration abgeschlossen haben, da dies Auswirkungen auf den NTP hat, wenn Sie <<step-6,Befehl „Neuimplementierung“>> auf dem Management-Node ausführen.
...
. [[step-6]]Den Befehl „management node redeploy“ konfigurieren und ausführen, um eine Verbindung zu persistenten Volumes herzustellen, die auf dem Cluster gehostet sind, und Dienste mit den vorherigen Konfigurationsdaten des Management-Knotens zu starten:
+
