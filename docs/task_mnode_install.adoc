...
... Fügen Sie für jeden Standardzeitserver, den Sie hinzufügen möchten, eine neue Zeile hinzu. Die Standardzeitserver müssen dieselben NTP-Server sein, die auf dem Speichercluster verwendet werden, das Sie in einer link:task_mnode_install.html#step-4-set-up-the-management-node["Später Schritt"] verwenden.
+
[listing]
----
vi /etc/ntp.conf
...
NOTE: Bearbeiten Sie den NTP nicht, nachdem Sie die Zeitsynchronisierungskonfiguration abgeschlossen haben, da dies Auswirkungen auf den NTP hat, wenn Sie link:task_mnode_install.html#step-4-set-up-the-management-node["Setup-Befehl"] auf dem Management-Node ausführen.
