...
* link:task_hcc_upgrade_compute_node_firmware.html["Die BMC-Firmware auf Ihrem Knoten aktualisieren"]
...
. Konfigurieren Sie Ihren BMC, siehe link:hci_prereqs_final_prep.html["Konfigurieren Sie IPMI für jeden Node"] Entsprechende Details.
. Lesen Sie den Artikel der NetApp Knowledge Base https://kb.netapp.com/onprem/solidfire/Element_OS/How_to_disable_BMC_web_UI_access_on_HCI_and_SolidFire_H-Series_systems["Deaktivieren des BMC Web UI-Zugriffs auf HCI und SolidFire H-Series Systeme"^] Um festzustellen, ob Ihr BMC-Webzugriff deaktiviert werden muss.
...
== Aktualisieren Sie die Firmware auf dem Node

Nach dem Austausch des Computing-Node müssen Sie eventuell die Firmware-Version aktualisieren. Siehe link:task_hcc_upgrade_compute_node_firmware.html["Aktualisiert die Firmware der Computing-Node"] Entsprechende Details.
